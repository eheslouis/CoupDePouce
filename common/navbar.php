<!-- ================ NAVBAR ================ -->
            <div class="row">
                <!-- <div class="col-md-2">
                    <a href="index.php"><img class="navbar-logo" src="images/logo.jpg" alt="logo coup de pouce"></a>
                </div> -->
                <div class="col-md-12">
                    <div class="navbar-wrapper">
                        <nav class="navbar navbar-default navbar-static-top">
                            <div class="navbar-header">
                                <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
                                    <span class="glyphicon glyphicon-menu-hamburger" aria-labe="menu"></span>
                                </button>
                                <a href="index.php"><img class="navbar-logo" src="images/logo.jpg" alt="logo coup de pouce"></a>
                            </div>
                            <div id="navbar" class="navbar-collapse collapse">
                                <ul class="nav navbar-nav">
                                    <li class="active"><a href="index.php">Accueil</a></li>
                                    <li><a href="assoc.php">Notre Association</a></li>
                                    <li class="dropdown">
                                        <a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Notre Actualité <b class="caret"></b></a>
                                        <ul class="dropdown-menu" role="menu">
                                            <li class="sub-item-menu"><a href="vieAssoc.php">La vie de l'association</a></li>
                                            <li class="sub-item-menu"><a href="agenda.php">Agenda</a></li>
                                            <li class="sub-item-menu"><a href="galeriePhoto.php">Galerie photo</a></li>
                                        </ul>
                                    </li>
                                    <li><a href="parrainage.php">Le Parrainage</a></li>

                                    <li class="dropdown">
                                        <a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Nos Projets <b class="caret"></b></a>
                                        <ul class="dropdown-menu" role="menu">
                                            <li class="dropdown-header">Soutien à nos familles parrainées</li>
                                            <li class="sub-item-menu"><a href="bourse.php">Bouse d'étude universitaire</a></li>
                                            <li class="sub-item-menu"><a href="habitat.php">Amélioration de l'habitat</a></li>
                                            <li class="divider"></li>
                                            <li class="dropdown-header">Autres projets</li>
                                            <li class="sub-item-menu"><a href="meovac.php">Meo-Vac</a></li>
                                            <li class="sub-item-menu"><a href="phuctue.php">Phuc Tue</a></li>
                                            <li class="sub-item-menu"><a href="bluedragon.php">Blue Dragon</a></li>
                                            <li class="sub-item-menu"><a href="sob.php">School on the Boat</a></li>
                                            <li class="divider"></li>
                                            <li><a href="archives.php">Archives</a></li>
                                        </ul>
                                    </li>
                                    <li class="dropdown">
                                        <a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">Votre Action <b class="caret"></b></a>
                                        <ul class="dropdown-menu" role="menu">
                                            <li class="sub-item-menu"><a href="participer.php">Comment participer</a></li>
                                            <li class="sub-item-menu"><a href="dons.php">Dons - Sponsor</a></li>
                                            <li class="sub-item-menu"><a href="boutique.php">Boutique</a></li>
                                        </ul>
                                    </li>

                                    <li class="social"><a href="https://www.facebook.com/coupdepoucevn" target="_blank" class="zocial-facebook"></a></li>
                                    <!-- <li class="social"><a href="#" class="zocial-twitter"></a></li> -->
                                    <li class="social"><a href="contact.php" class="zocial-email"></a></li>
                                </ul>
                            </div>
                        </nav>
                    </div>
                </div>
            </div>
